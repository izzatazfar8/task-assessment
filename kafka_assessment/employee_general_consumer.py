...
# Kafka conf. for general messages
consumer_config = {
    'bootstrap.servers': 'localhost:9092',      
    'group.id': 'new_employees_general',         
    'auto.offset.reset': 'earliest'             
}

# Initialize the Kafka consumer 
consumer = Consumer(consumer_config)
consumer.subscribe(['general_messages'])

# Load previously processed messages (using content hashes) from a file
try:
    with open('/home/izzat/processed_general_messages.json', 'r') as f:
...
try:
    while True:
        ...
        if message_hash not in processed_messages:
            print(f"New message in general_messages: {message_content}")
            processed_messages[message_hash] = True

            # Save the processed messages to the file
            with open('/home/izzat/processed_general_messages.json', 'w') as f:
                json.dump(processed_messages, f)

except KeyboardInterrupt:
    print("Exiting general messages consumer...")
